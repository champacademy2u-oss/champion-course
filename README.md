...
9. 在 Leads 页面先勾选客户，再点击 `Bulk WhatsApp (Selected)`。系统会先显示脱敏名单预览、自动把马来西亚本地 `01...` 号码转换为 `601...`、排除无效与重复号码，并允许在发送前自定义文案和选择一张本机图片。确认客户已同意接收信息后，每批最多打开 10 个 WhatsApp 聊天。系统会填入文案，但 WhatsApp 链接不能自动附图；如有图片，先在预览中复制，再到每个聊天粘贴并由管理员亲自检查、按发送。
