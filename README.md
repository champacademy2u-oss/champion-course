...
9. 在 Leads 页面先勾选客户，再点击 `Bulk WhatsApp (Selected)`。系统会先显示脱敏名单预览、自动把马来西亚本地 `01...` 号码转换为 `601...`、排除无效与重复号码，并允许在发送前自定义文案和选择一张本机图片。文案编辑器会保留完整 Emoji、提供常用 Emoji 快捷按钮，并在发现损坏字符 `�` 时禁止打开聊天，直到管理员清除并重新插入正确 Emoji。WhatsApp 链接直接使用官方 `api.whatsapp.com/send`，避免 `wa.me` 中转重定向把 Emoji 转成 `�`。确认客户已同意接收信息后，每批最多打开 10 个 WhatsApp 聊天。系统会填入文案，但 WhatsApp 链接不能自动附图；如有图片，先在预览中复制，再到每个聊天粘贴并由管理员亲自检查、按发送。
...
2. 填写 Campaign 名称、标题、预览文字、正文、CTA 按钮和 HTTPS 链接；可上传一张不超过 50 MB 的 JPG/PNG/WebP 图片，或填写公开的 HTTPS 图片链接，再保存草稿。图片会显示在正文与 CTA 按钮之间；部分收件箱可能阻挡远端图片，因此正文不能只靠图片传达重要信息。大图片可能载入较慢，建议尽量压缩。
3. 先寄到固定的管理员测试邮箱。
...
更换或移除图片会改变 Campaign 内容版本，必须重新保存并寄管理员测试邮件。浏览器把图片分成每块 2 MB，经管理员认证的 API 写入私有 R2，服务器检查格式后合并；因此整张图片可以超过 Vercel Function 的单次请求限制，也不需要额外的 R2 浏览器 CORS 设置。邮件通过公开只读、串流传送的图片地址载入。请勿上传包含客户资料或机密内容的图片。图片 URL 不计入 CTA 点击报告。
