...
export function mergeAudienceSelections(current = [], additions = []) {
  const groups = new Map();
  const normalizedCurrent = Array.isArray(current) && current.length ? validateAudienceSelections(current) : [];
  for (const selection of [...normalizedCurrent, ...validateAudienceSelections(additions)]) {
    if (!groups.has(selection.source)) groups.set(selection.source, new Set());
    selection.ids.forEach(id => groups.get(selection.source).add(id));
  }
  return validateAudienceSelections(Array.from(groups, ([source, ids]) => ({ source, ids: Array.from(ids) })));
}

export function mergeAudienceStats(current = {}, incremental = {}) {
  const sumFields = ['selected', 'valid', 'excluded', 'invalid', 'duplicate', 'suppressed', 'noConsent', 'missing', 'alreadyAdded'];
  return Object.fromEntries(sumFields.map(field => [
    field,
    (Number(current[field]) || 0) + (Number(incremental[field]) || 0)
  ]));
}
...
export function sanitizeCampaignInput(input = {}) {
  ...
  const campaign = {
    ...
    imageUrl: imageUrl ? validateHttpsUrl(imageUrl, '图片链接') : '',
    imageAlt: imageUrl ? cleanText(input.imageAlt, 160) || 'Champion Academy 课程图片' : '',
...
export function buildEmailContent({ campaign, recipient, unsubscribeUrl, ctaUrl = '', openPixelUrl = '' }) {
  ...
  const imageAlt = applyNameTemplate(campaign.imageAlt || 'Champion Academy 课程图片', name);
  const imageUrl = cleanText(campaign.imageUrl, 2048);
  const footerUrl = cleanText(unsubscribeUrl, 2400);
  const ctaTarget = cleanText(ctaUrl, 2400) || campaign.ctaUrl;
  const pixel = cleanText(openPixelUrl, 2400)
    ? `<img src="${escapeHtml(openPixelUrl)}" width="1" height="1" alt="" style="display:block;width:1px;height:1px;border:0" />`
    : '';
  const image = imageUrl
    ? `<div style="margin:24px 0"><img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(imageAlt)}" width="554" style="display:block;width:100%;max-width:554px;height:auto;border:0;border-radius:10px" /></div>`
    : '';
  const html = `<!doctype html><html><body style="margin:0;background:#f3f5f7;color:#18212b;font-family:Arial,sans-serif"><div style="display:none;max-height:0;overflow:hidden;opacity:0">${escapeHtml(previewText)}</div><div style="max-width:620px;margin:0 auto;padding:28px 16px"><div style="background:#ffffff;border:1px solid #e6e9ed;border-radius:14px;padding:32px"><div style="font-size:20px;font-weight:800;color:#18212b;margin-bottom:24px">Champion Academy</div><div style="font-size:16px;line-height:1.75">${textToEmailHtml(bodyText)}</div>${image}<div style="margin:28px 0"><a href="${escapeHtml(ctaTarget)}" style="display:inline-block;background:#e8590c;color:#ffffff;text-decoration:none;font-weight:700;padding:13px 22px;border-radius:8px">${escapeHtml(ctaLabel)}</a></div><hr style="border:0;border-top:1px solid #eceff2;margin:30px 0 18px"><p style="margin:0;color:#6b7280;font-size:12px;line-height:1.6">您收到这封邮件，是因为您曾同意接收 Champion Academy 的通知。<br><a href="${escapeHtml(footerUrl)}" style="color:#6b7280">取消订阅未来 Email</a></p>${pixel}</div></div></body></html>`;
...
function trackingKey(secret) {
  const value = cleanText(secret, 1000);
  if (!value) throw new Error('EMAIL_TRACKING_SECRET 尚未配置');
  return crypto.createHmac('sha256', Buffer.from(value, 'utf8')).update('champion-email-tracking-v1').digest();
}

export function createEmailTrackingToken(payload, secret) {
  const campaignId = cleanText(payload?.campaignId, 180);
  const recipientId = cleanText(payload?.recipientId, 180);
  const kind = cleanText(payload?.kind, 20);
  if (!campaignId || !recipientId || !['open', 'click'].includes(kind)) throw new Error('追踪资料不完整');
  const encoded = Buffer.from(JSON.stringify({ campaignId, recipientId, kind })).toString('base64url');
  const signature = crypto.createHmac('sha256', trackingKey(secret)).update(encoded).digest('base64url');
  return `${encoded}.${signature}`;
}

export function verifyEmailTrackingToken(token, secret, expectedKind = '') {
  const [encoded, signature] = cleanText(token, 4000).split('.');
  if (!encoded || !signature) throw new Error('追踪链接无效');
  const expected = crypto.createHmac('sha256', trackingKey(secret)).update(encoded).digest('base64url');
  if (!timingSafeStringEqual(signature, expected)) throw new Error('追踪链接无效');
  try {
    const payload = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8'));
    const campaignId = cleanText(payload.campaignId, 180);
    const recipientId = cleanText(payload.recipientId, 180);
    const kind = cleanText(payload.kind, 20);
    if (!campaignId || !recipientId || !['open', 'click'].includes(kind)) throw new Error();
    if (campaignId.includes('/') || recipientId.includes('/')) throw new Error();
    if (expectedKind && kind !== expectedKind) throw new Error();
    return { campaignId, recipientId, kind };
  } catch {
    throw new Error('追踪链接无效');
  }
}
