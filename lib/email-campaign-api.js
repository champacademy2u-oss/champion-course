...
import { deleteStoredObject, getStoredObject, putStoredObject } from '../api/_r2.js';
import { campaignImageMatchesType, decodeCampaignImage, decodeCampaignImageChunk, EMAIL_IMAGE_CHUNK_BYTES, expectedCampaignImageChunks, MAX_EMAIL_IMAGE_BYTES, validEmailImageId, validateCampaignImageUpload } from './email-campaign-image.js';
...
function errorStatus(error) {
  if (['未授权', 'Email 管理员尚未配置', '此账号没有 Email 管理权限'].includes(error.message)) return 401;
...
async function requireCrmAdmin(req) {
  ...
  const allowed = new Set(String(process.env.CRM_ADMIN_UIDS || '')
    .split(',').map(value => value.trim()).filter(Boolean));
  const allowedEmails = new Set(String(process.env.CRM_ADMIN_EMAILS || '')
    .split(',').map(normalizeEmail).filter(Boolean));
  if (!allowed.size && !allowedEmails.size) throw new Error('Email 管理员尚未配置');
  try {
    const decoded = await firebaseApp().auth().verifyIdToken(token);
    const email = normalizeEmail(decoded.email);
    const isVerifiedGoogleAdmin = decoded.email_verified === true
      && decoded.firebase?.sign_in_provider === 'google.com'
      && allowedEmails.has(email);
    if (!allowed.has(decoded.uid) && !isVerifiedGoogleAdmin) throw new Error();
    return decoded;
  } catch {
...
async function uploadCampaignImage(input) {
  const { bytes, contentType } = decodeCampaignImage(input);
  const baseUrl = publicApiBase();
  const imageId = crypto.randomUUID();
  await putStoredObject(`email-images/${imageId}`, bytes, contentType);
  return { imageUrl: `${baseUrl}/api/email-image?id=${imageId}` };
}

async function prepareCampaignImageUpload(input) {
  validateCampaignImageUpload(input);
  const imageId = crypto.randomUUID();
  return { imageId, chunkBytes: EMAIL_IMAGE_CHUNK_BYTES };
}

async function uploadCampaignImageChunk(input) {
  const imageId = String(input.imageId || '');
  const index = Number(input.index);
  if (!validEmailImageId(imageId) || !Number.isSafeInteger(index) || index < 0 || index >= expectedCampaignImageChunks(MAX_EMAIL_IMAGE_BYTES)) {
    throw new Error('图片分块编号不正确');
  }
  const bytes = decodeCampaignImageChunk(input.data);
  await putStoredObject(`email-image-uploads/${imageId}/${index}`, bytes, 'application/octet-stream');
  return { uploaded: index };
}

async function finishCampaignImageUpload(input) {
  const imageId = String(input.imageId || '');
  if (!validEmailImageId(imageId)) throw new Error('图片上传编号不正确');
  const { contentType, size } = validateCampaignImageUpload(input);
  const count = expectedCampaignImageChunks(size);
  const chunks = [];
  for (let index = 0; index < count; index += 1) {
    let object;
    try {
      object = await getStoredObject(`email-image-uploads/${imageId}/${index}`);
    } catch (error) {
      if (error?.name === 'NotFound' || error?.name === 'NoSuchKey' || error?.$metadata?.httpStatusCode === 404) {
        throw new Error('图片尚未上传完成，请重新选择文件');
      }
      throw error;
    }
    const expectedSize = Math.min(EMAIL_IMAGE_CHUNK_BYTES, size - index * EMAIL_IMAGE_CHUNK_BYTES);
    if (Number(object.ContentLength) !== expectedSize) throw new Error('图片分块大小不正确，请重新选择文件');
    const partChunks = [];
    let partSize = 0;
    for await (const chunk of object.Body) {
      partSize += chunk.length;
      if (partSize > expectedSize) throw new Error('图片分块大小不正确，请重新选择文件');
      partChunks.push(chunk);
    }
    if (partSize !== expectedSize) throw new Error('图片分块大小不正确，请重新选择文件');
    chunks.push(Buffer.concat(partChunks, expectedSize));
  }
  const bytes = Buffer.concat(chunks, size);
  if (!campaignImageMatchesType(contentType, bytes.subarray(0, 12))) throw new Error('图片内容与格式不符');
  await putStoredObject(`email-images/${imageId}`, bytes, contentType);
  await Promise.allSettled(Array.from({ length: count }, (_, index) => deleteStoredObject(`email-image-uploads/${imageId}/${index}`)));
  return { imageUrl: `${publicApiBase()}/api/email-image?id=${imageId}` };
}
...
export async function handleEmailCampaignRequest(req, res) {
  ...
  try {
    ...
    else if (req.method === 'POST' && action === 'upload-image') result = await uploadCampaignImage(input);
    else if (req.method === 'POST' && action === 'prepare-image-upload') result = await prepareCampaignImageUpload(input);
    else if (req.method === 'POST' && action === 'upload-image-chunk') result = await uploadCampaignImageChunk(input);
    else if (req.method === 'POST' && action === 'finish-image-upload') result = await finishCampaignImageUpload(input);
