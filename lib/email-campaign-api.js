...
async function requireCrmAdmin(req) {
  ...
  try {
    ...
    throw new Error('此浏览器没有 Email 管理权限');
  }
}
...
function configuredEmailFrom() {
  const value = requiredEnv('EMAIL_FROM', 'EMAIL_FROM 尚未配置');
  if (!/^Champion Academy <updates@[a-z0-9.-]+\.[a-z]{2,}>$/i.test(value)) {
    throw new Error('EMAIL_FROM 必须使用 Champion Academy <updates@已验证域名>');
  }
  return value;
}
...
async function sendThroughResend({ to, subject, html, text, idempotencyKey, listUnsubscribe }) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      return { id: cleanText(data.id, 240), attempt };
    } catch (error) {
...
async function sendTest(input) {
  ...
  return { ok: true, sentAt: timestamp, recipient: testRecipient };
}
...
async function startCampaign(input, admin) {
  ...
  if (!campaign.testSentAt || campaign.testSentContentVersion !== campaign.contentVersion) {
    throw new Error('请先发送当前内容的测试邮件');
  }
...
async function sendRecipient(campaign, recipient) {
  ...
  const content = buildEmailContent({ campaign, recipient, unsubscribeUrl: url });
  try {
    await reserveSendSlot();
    const result = await sendThroughResend({
      to: recipient.email,
    ...
    batch.set(db().collection(EMAIL_MESSAGES).doc(result.id), {
...
export async function handleEmailCampaignRequest(req, res) {
  ...
  try {
    ...
    const input = req.method === 'GET'
      ...
      : parseBody(req);
    let result;
    if (req.method === 'GET' && action === 'list') result = { campaigns: await listCampaigns() };
...
function renderUnsubscribePage(res, status, title, message) {
...
export async function handleEmailUnsubscribeRequest(req, res) {
  if (!['GET', 'POST'].includes(req.method)) {
    return renderUnsubscribePage(res, 405, '无法处理', '这个链接只支持 Email 退订操作。');
  }
  const url = new URL(req.url, 'https://local');
  if (url.searchParams.get('preview') === '1') {
    return renderUnsubscribePage(res, 200, '测试邮件预览', '这是测试邮件中的退订链接，不会修改任何客户资料。');
  }
  try {
    ...
    await batch.commit();
    return renderUnsubscribePage(res, 200, '已取消订阅', '您不会再收到 Champion Academy 未来的 Email Campaign。');
  } catch {
    return renderUnsubscribePage(res, 400, '链接无效', '这个退订链接无效或已经无法使用，请直接联络 Champion Academy。');
  }
}
